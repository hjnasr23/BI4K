...
function UploadContent() {
  ...
  return (
    <div className="min-h-screen bg-neutral-50 dark:bg-[#0a0a0c] text-neutral-900 dark:text-white transition-colors duration-500 flex flex-col">
      <Navbar />

      <main className="flex-grow pt-24 pb-12 flex flex-col lg:flex-row bg-neutral-50 dark:bg-[#0a0a0c] container mx-auto px-4 lg:px-8 gap-8">
        
        {/* Expanded Canvas Stage (Front and Center) */}
        <div className="flex-grow flex bg-white dark:bg-[#111116] border border-neutral-200 dark:border-white/5 rounded-[2.5rem] overflow-hidden shadow-sm dark:shadow-2xl relative">
          <div className="flex-grow relative bg-neutral-100 dark:bg-[#0a0a0c] flex items-center justify-center p-8 overflow-hidden">
            ...
            <div className="relative w-full max-w-[550px] aspect-[4/5] flex items-center justify-center animate-reveal">
              ...
              <div className="absolute inset-0 border border-neutral-200 dark:border-white/5 rounded-[3rem] pointer-events-none" />

              {/* Product Mockup Container (Fixed 500x500 reference scale) */}
              <div className="relative w-[500px] h-[500px] flex items-center justify-center rounded-2xl overflow-hidden bg-white dark:bg-black shadow-[0_40px_80px_rgba(0,0,0,0.1)] dark:shadow-[0_40px_80px_rgba(0,0,0,0.6)]">
                ...
                {queryMockupUrl ? (
                  ...
                  <div className="absolute inset-0 bg-neutral-200 dark:bg-[#111116] flex items-center justify-center">
                    <span className="text-neutral-400 dark:text-foreground/20 font-black tracking-widest text-xs uppercase">No Mockup loaded</span>
                  </div>
                )}

                {/* Print Zone Guide Overlay */}
                <div className="absolute border border-dashed border-neutral-300 dark:border-white/20 pointer-events-none rounded-xl z-20" style={{ left: 125, top: 100, width: 250, height: 280 }}>
                  <div className="absolute -top-1 -left-1 w-2 h-2 bg-neutral-400 dark:bg-white/50 rounded-full" />
                  <div className="absolute -top-1 -right-1 w-2 h-2 bg-neutral-400 dark:bg-white/50 rounded-full" />
                  <div className="absolute -bottom-1 -left-1 w-2 h-2 bg-neutral-400 dark:bg-white/50 rounded-full" />
                  <div className="absolute -bottom-1 -right-1 w-2 h-2 bg-neutral-400 dark:bg-white/50 rounded-full" />
                  <span className="absolute -top-5 left-1/2 -translate-x-1/2 text-[8px] font-black uppercase tracking-widest text-neutral-400 dark:text-white/40 whitespace-nowrap">Zone d'impression</span>
                </div>
                ...
                {isStandardizing && (
                  <div className="absolute inset-0 z-50 flex flex-col items-center justify-center bg-white/80 dark:bg-black/75 backdrop-blur-sm pointer-events-auto rounded-2xl">
                    <Loader2 className="w-10 h-10 animate-spin text-brand-blue mb-4" />
                    <p className="font-black uppercase text-[10px] tracking-[0.25em] text-neutral-900 dark:text-white">Optimisation du design...</p>
                    <p className="text-[8px] text-neutral-500 dark:text-foreground/40 font-bold uppercase tracking-[0.2em] mt-1.5">Standardisation en 1024x1024 PNG</p>
                  </div>
                )}
                ...
                {!designUrl && !isStandardizing && (
                  <div className="absolute inset-0 z-30 flex items-center justify-center pointer-events-auto">
                    <label className="flex flex-col items-center justify-center p-6 border border-dashed border-neutral-300 dark:border-white/10 rounded-2xl bg-white/70 dark:bg-black/60 backdrop-blur-md hover:border-brand-blue dark:hover:border-brand-blue transition-all cursor-pointer group text-center max-w-[240px] shadow-lg dark:shadow-none">
                      ...
                      <p className="font-black uppercase text-[9px] tracking-widest mb-0.5 text-neutral-900 dark:text-white">Importer le design</p>
                      <p className="text-[7px] text-neutral-500 dark:text-neutral-400 font-bold uppercase tracking-[0.2em] mb-2">Format PNG/JPG</p>
                      <p className="text-[8px] font-bold text-neutral-600 dark:text-amber-400/90 leading-relaxed uppercase tracking-wider">
                        Astuce : Utilisez un fond transparent
                      </p>
                    </label>
                  </div>
                )}
        ...
        <aside className="w-full lg:w-[350px] space-y-6 shrink-0">
          <div className="bg-white dark:bg-[#111116] border border-neutral-200 dark:border-white/5 rounded-[2.5rem] p-8 shadow-sm dark:shadow-2xl animate-reveal">
            <h3 className="text-xs font-black uppercase tracking-[0.4em] text-brand-blue dark:text-brand-yellow mb-8 flex items-center gap-3">
              <div className="w-1.5 h-1.5 rounded-full bg-brand-blue dark:bg-brand-yellow animate-pulse" />
              Order Summary
            </h3>
            ...
            {dbLoading ? (
              ...
              <div className="space-y-6 mb-8">
                <div className="flex justify-between items-center">
                  <p className="text-[10px] font-bold text-neutral-400 dark:text-foreground/40 uppercase tracking-widest">Base Product</p>
                  <p className="text-[10px] font-black uppercase tracking-tighter text-neutral-900 dark:text-white">{productData?.name}</p>
                </div>
                <div className="flex justify-between items-center">
                  <p className="text-[10px] font-bold text-neutral-400 dark:text-foreground/40 uppercase tracking-widest">Selected Size</p>
                  <p className="text-[10px] font-black uppercase tracking-tighter text-neutral-900 dark:text-white">{querySize}</p>
                </div>
                 <div className="flex justify-between items-center">
                  <p className="text-[10px] font-bold text-neutral-400 dark:text-foreground/40 uppercase tracking-widest">Unit Price</p>
                  {hasValidDiscount ? (
                    ...
                    <p className="text-lg font-black text-brand-blue dark:text-brand-yellow italic">{currentPrice} MAD</p>
                  )}
                </div>
              </div>
            )}
            ...
            {designFile && !isStandardizing && (
              <div className="border-t border-neutral-200 dark:border-white/5 pt-6 mb-8 space-y-5">
                <div className="flex items-center justify-between p-3.5 rounded-xl bg-neutral-50 dark:bg-white/5 border border-neutral-200 dark:border-white/5">
                  <span className="text-[9px] font-bold text-neutral-700 dark:text-slate-300 truncate max-w-[160px] uppercase tracking-wider">{designFile.name}</span>
                  <button onClick={() => { setDesignFile(null); setDesignUrl(null); if (fabricRef.current) fabricRef.current.clear(); }} className="text-red-500 dark:text-rose-400 hover:text-red-400 dark:hover:text-rose-300 transition-colors text-[9px] font-black uppercase tracking-widest">
                    Remove
                  </button>
                </div>

                <div className="space-y-3">
                  <button onClick={handleResetAlignment} className="w-full py-2 rounded-xl bg-neutral-50 hover:bg-neutral-100 dark:bg-white/5 dark:hover:bg-white/10 text-[8px] font-black uppercase tracking-widest border border-neutral-200 dark:border-white/5 text-neutral-500 dark:text-slate-400 transition-colors">
                    Reset Alignment
                  </button>
                </div>
              </div>
            ...
            {/* Quantity */}
            <div className="space-y-3 mb-8 border-t border-neutral-200 dark:border-white/5 pt-6">
              <label className="text-[9px] font-black uppercase tracking-[0.2em] text-neutral-400 dark:text-foreground/40">Total Quantity</label>
              <div className="flex items-center gap-4 bg-neutral-100 dark:bg-white/5 border border-neutral-200 dark:border-white/5 rounded-2xl p-1.5">
                <button onClick={() => setQuantity(q => Math.max(1, q - 1))} className="w-9 h-9 rounded-xl hover:bg-neutral-200 dark:hover:bg-white/10 flex items-center justify-center transition-colors font-bold text-sm text-neutral-800 dark:text-white">-</button>
                <span className="flex-grow text-center font-black text-xs text-neutral-900 dark:text-white">{quantity}</span>
                <button onClick={() => setQuantity(q => q + 1)} className="w-9 h-9 rounded-xl hover:bg-neutral-200 dark:hover:bg-white/10 flex items-center justify-center transition-colors font-bold text-sm text-neutral-800 dark:text-white">+</button>
              </div>
            </div>

             {/* Total */}
            <div className="pt-6 border-t border-neutral-200 dark:border-white/5 mb-8">
              <div className="flex justify-between items-end">
                <p className="text-[9px] font-black uppercase tracking-[0.4em] text-neutral-300 dark:text-foreground/20">Final Amount</p>
                <div className="text-right">
                  ...
                  <p className="text-3xl font-black tracking-tighter text-neutral-900 dark:text-white">{((currentPrice * quantity) % 1 === 0 ? (currentPrice * quantity) : (currentPrice * quantity).toFixed(2))} MAD</p>
                </div>
              </div>
            </div>
            ...
            <button
              ...
              className={`w-full py-5 rounded-2xl flex items-center justify-center gap-3 font-black text-xs uppercase tracking-[0.3em] transition-all relative overflow-hidden group/order ${!designFile || isFinishing || isStandardizing ? 'bg-neutral-100 dark:bg-white/5 text-neutral-400 dark:text-foreground/20 cursor-not-allowed border border-neutral-200 dark:border-white/10' : 'bg-brand-blue text-white hover:scale-[1.02] active:scale-[0.98] shadow-xl shadow-brand-blue/20'}`}
            >
          ...
          <div className="bg-brand-blue/5 border border-brand-blue/20 rounded-3xl p-5 flex items-start gap-3">
            <Sparkles className="w-4 h-4 text-brand-blue dark:text-brand-yellow shrink-0 mt-0.5 animate-pulse" />
            <p className="text-[9px] font-bold text-brand-blue dark:text-brand-yellow leading-relaxed uppercase tracking-wider">
      ...
      {showToast && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-md z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-[#111116] border border-neutral-200 dark:border-white/10 rounded-[2.5rem] p-8 max-w-md w-full shadow-2xl text-center animate-reveal">
...
export default function UploadPage() {
  return (
    <Suspense fallback={
      <div className="min-h-screen bg-neutral-50 dark:bg-[#0a0a0c] flex flex-col justify-center items-center gap-4 text-neutral-900 dark:text-white">
